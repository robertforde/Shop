...
/**
 * This class represents trade order objects which are held in the database in both the orderheader and the orderdetail tables. It extends the Order class 
 * @author Robert Forde
 *
 */
public class TradeOrder extends Order implements Printable{
	...
	/**
	 * Pre-printing method that ensures the arraylists are updated and that saves the trade order or quote to the appropriate tables.
	...
	public void printOrder(InvoiceSettings invoiceSettings, AccountDetailScreen accountDetailScreen, AccountListScreen accountListScreen, Account acc) throws Exception{
		...
		if(this.getReceiptNo() == 0) {
			...
			int line = 0;
			for(TradeOrderLine orderLine : this.getOrderList()){
				orderLine.setReceiptNo(receiptNo);
				orderLine.setLineNo(++line);
				getOrderDao().insertTradeDetailOrder(typeNo, detailTable, orderLine);
			}
		...
		if(this.getSaleType().equals("Trade") || this.getSaleType().equals("TradeQuote")) {
			...
			if(getCustId()==0 && (!getName().isEmpty() || !getAddress1().isEmpty() || !getAddress2().isEmpty() || !getPhone().isEmpty()) ){
				...
				Tradesman tradesman = new Tradesman(getName(), getAddress1(), getAddress2(), getPhone(), 0);
				getTradesmanDao().tradesmanInsert(tradesman);
							
			}else{

				Tradesman tradesman = new Tradesman(getCustId(), getName(), getAddress1(), getAddress2(), getPhone(), 0);
				getTradesmanDao().tradesmanUpdate(tradesman);
			...
			balance = getAccountDao().accountRetrieveBalance(this.getCustId());
			...
			getAccountDao().accountUpdateBalance(this.getCustId(), balance);
	...
	/**
	 * Method to re-calculate order totals and the gross profit value and percentage after a trade order line has been deleted from an order
	 * @param vatRate The vat rate of the order
	 * @param lblGrossProfitValue The label that contains the gross profit value of the order
	 * @param lblGrossProfitPercent The label that contains the gross profit percent of the order
	 */
	...
	/**
	 * Method to calculate an orders gross profit value and gross profit percent
